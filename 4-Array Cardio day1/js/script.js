...
const year = inventors.filter(inventor => inventor.year >= 1500 && inventor.year <= 1599)
console.table(year)
...
const sequence = inventors.sort((a, b)  =>  a.year - b.year)
console.table(sequence)

// Array.prototype.reduce()
// 4. How many years did all the inventors live all together?

const liveAll = inventors.reduce((acc, inventor) => {
    return acc + (inventor.passed - inventor.year)
}, 0) //0 é valor inicial que será usado no parâmetro acc para a primeira soma feita. Senão ele começa como undefined.
console.log(liveAll)
...
const links = Array.from(category.querySelectorAll('a')) //Método para transformar a NodeList em um array e conseguirmos utilizar os métodos map() e filter()

const de = links.map(link => link.textContent) //Pega o conteúdo de cada link que são os nomes das ruas
